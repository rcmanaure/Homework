...
def service_area_center():
    return models.ServiceArea(kilometer=60, gas_price=894)


@pytest.fixture
def service_area_left():
    return models.ServiceArea(kilometer=40, gas_price=894)


@pytest.fixture
def service_area_right():
    return models.ServiceArea(kilometer=90, gas_price=894)
...
class TestServiceArea:
    def test_wrong_left_placement(self, service_area_center, service_area_right):
        ...
        assert not station.left_station == service_area_right

        with pytest.raises(ValidationError):
            service_area_center.clean()
            raise ValidationError("wrong placement")

    def test_wrong_right_placement(self, service_area_center, service_area_left):
        station = service_area_center
        assert not station.right_station == service_area_left
